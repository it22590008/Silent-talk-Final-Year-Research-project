...
class _EmergencyScreenState extends State<EmergencyScreen> {
  ...
  Future<void> _loadModel() async {
    _modelLoaded = await _yoloProcessor.loadModel(
      labelsPath: 'assets/models/emerg_labels.txt',
      modelPath: 'assets/models/emerg_float32.tflite',
      modelVersion: "yolov8",
    );
    setState(() {});
  }
  ...
  Future<void> _runDetection(CameraImage image) async {
    try {
      ...
      if (results != null && results.isNotEmpty) {
        ...
        final detectedTag = results.first['tag'];
        final message = _getEmergencyMessage(detectedTag);

        if (!_smsSent && message.isNotEmpty) {
          _smsSent = true;
  ...
  //  MESSAGE MAP 
  String _getEmergencyMessage(String tag) {
    switch (tag.toUpperCase()) {
      case "ACCIDENT":
        return "ACCIDENT\nI have been in an accident and need immediate assistance. Please respond as soon as possible.";

      case "CALL":
        return "CALL\nPlease call me immediately. This is urgent.";

      case "HELP":
        return "HELP\nI need help right now. Please reach me or contact emergency services.";

      case "HOT":
        return "HOT\nThere is a dangerous heat or fire-related situation. Please take immediate action.";

      case "LOSE":
        return "LOST\nI am lost and unable to communicate verbally. Please help me find assistance.";

      case "PAIN":
        return "PAIN\nI am in pain and may need medical attention. Please help me as soon as possible.";

      case "THIEF":
        return "THIEF\nThere is a theft or suspicious person nearby. Please contact security or authorities.";
  ...
  Future<void> _sendEmergencySMS(String message) async {
    final Uri uri = Uri.parse(
      "sms:0760171533?body=${Uri.encodeComponent(message)}",
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Emergency Sign Detection"),
...
class BoundingBoxPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var det in detections) {
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(x + 6, math.max(0, y - 20)));
    }
  }
